import{r as y,j as t}from"./radix-1h7f9VDF.js";import{S as k}from"./section-title-DgXbaEFO.js";import{u as E}from"./i18n-Blv0dhYd.js";import{L as D,d as T,a as b}from"./motion-eszmwozl.js";import"./index-B2bJhwY-.js";function C(){const r=y.useRef(null),p=y.useRef([]),l=y.useRef();return y.useEffect(()=>{const c=r.current;if(!c)return;const s=c.getContext("2d");if(!s)return;let e=0,a=0;const d=.5,o=150,f=60;let u=!1;const m=()=>{const i=c.parentElement;if(i){e=c.width=i.offsetWidth,a=c.height=i.offsetHeight,p.current=[];for(let n=0;n<f;n++)p.current.push({x:Math.random()*e,y:Math.random()*a,vx:(Math.random()-.5)*d,vy:(Math.random()-.5)*d})}};m(),window.addEventListener("resize",m);const g=(i,n)=>{const h=i.x-n.x,x=i.y-n.y;return Math.sqrt(h*h+x*x)},v=()=>{const i=p.current;for(let n=0;n<i.length-1;n++)for(let h=n+1;h<i.length;h++){const x=g(i[n],i[h]);if(x<=o){const S=1-x/o;s.strokeStyle=`hsla(177, 100%, 37%, ${S})`,s.lineWidth=Math.pow(1-x/o,1.5)*2,s.beginPath(),s.moveTo(i[n].x,i[n].y),s.lineTo(i[h].x,i[h].y),s.stroke()}}},w=()=>{const i=p.current;for(const n of i)s.beginPath(),s.arc(n.x,n.y,2,0,Math.PI*2),s.fillStyle="hsl(177, 100%, 47%)",s.fill()},M=()=>{const i=p.current;for(const n of i)n.x+=n.vx,n.y+=n.vy,(n.x<0||n.x>e)&&(n.vx*=-1),(n.y<0||n.y>a)&&(n.vy*=-1)},j=()=>{u&&(s.clearRect(0,0,e,a),v(),w(),M(),l.current=requestAnimationFrame(j))},N=new IntersectionObserver(([i])=>{u=i.isIntersecting,u?l.current=requestAnimationFrame(j):l.current&&cancelAnimationFrame(l.current)},{threshold:0});return N.observe(c),()=>{N.disconnect(),window.removeEventListener("resize",m),l.current&&cancelAnimationFrame(l.current)}},[]),t.jsx("canvas",{ref:r,className:"absolute inset-0 w-full h-full"})}const P=`// Get the canvas element and set up the 2D context
